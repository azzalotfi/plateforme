...
class User implements UserInterface, PasswordAuthenticatedUserInterface
{
    ...
    // Gains totaux (colonne total_gains)
    #[ORM\Column(type: 'float', nullable: true, name: 'total_gains', options: ['default' => 0])]
    private ?float $totalGains = 0.0;

    // Prestations complétées
    #[ORM\Column(type: 'integer', nullable: true, options: ['default' => 0])]
    private ?int $completed = 0;

    // Nombre d'avis (colonne reviews_count)
    #[ORM\Column(type: 'integer', nullable: true, name: 'reviews_count', options: ['default' => 0])]
...
public function setIsVerified(bool $isVerified): self
{
    ...
    {
        // Tous les utilisateurs authentifiés ont ROLE_USER
        $roles = ['ROLE_USER'];
        
        // Ajouter le rôle spécifique avec le préfixe ROLE_
        if (!empty($this->role)) {
            $roleName = strtoupper(trim($this->role));
            // Ajouter ROLE_ si ce n'est pas déjà présent
    ...
    public function getTotalGains(): ?float { return $this->totalGains; }
    public function setTotalGains(?float $totalGains): self { $this->totalGains = $totalGains; return $this; }
